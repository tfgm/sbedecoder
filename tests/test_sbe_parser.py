...
class TestSBEParserLibrary:
    ...
    def setup(self):
        self.schema = SBESchema()
        self.schema.parse('./tests/templates_FixBinary.xml')
        self.msg_factory = SBEMessageFactory(self.schema)
        self.parser = SBEParser(self.msg_factory)
    ...
    def test_security_status_reset_statistics(self):
        ...
        for message in self.parser.parse(msg_buffer, offset):
